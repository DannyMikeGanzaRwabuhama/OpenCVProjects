...
if not ok or not decoded_info or not any(decoded_info):
    sys.exit("Error: No barcode detected. Try better lighting or closer photo.")

print("\nDecoded Barcode(s):")
print("-" * 60)
for i, text in enumerate(decoded_info):
    if text:
        print(f"[{i+1}] {text}  →  Type: {decoded_type[i] if decoded_type else 'Unknown'}")
print("-" * 60)
...
first_text = next((t for t in decoded_info if t), "Unknown")
cv2.putText(
    img_color,
    f"{first_text}",
    (10, 30),
...
cv2.imshow("Detected Barcode", img_color)
